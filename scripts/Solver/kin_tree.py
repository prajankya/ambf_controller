...
class Chain:
    ...
    def __init__(self, yaml_file_data):
        ...
        for body in self._bodies.items():
            print(body)
            if not self._add_children_to_tree(body, tree):
                ok = False
                break

        return (ok, tree)

    def _toKdlPose(self, pose):
        # URDF might have RPY OR XYZ unspecified. Both default to zeros
        rpy = pose.rpy if pose and pose.rpy and len(
            pose.rpy) == 3 else [0, 0, 0]
        xyz = pose.xyz if pose and pose.xyz and len(
            pose.xyz) == 3 else [0, 0, 0]

        return kdl.Frame(
            kdl.Rotation.RPY(*rpy),
            kdl.Vector(*xyz))

    def _toKdlInertia(self, i):
        ...
        origin = self._toKdlPose(i.origin)
        inertia = i.inertia
    ...
    def _toKdlJoint(self, jnt):
        ...
        return type_map[jnt.type](jnt, self._toKdlPose(jnt.origin))

    def _add_children_to_tree(self, root, tree):
        # None robot_model

        # constructs the optional inertia
        inert = kdl.RigidBodyInertia(0)
        if root.inertial:
            inert = self._toKdlInertia(root.inertial)
...
# URDF might have RPY OR XYZ unspecified. Both default to zeros
        ...
        # # TODO : How to do this for multiple parents
        # # constructs the kdl joint
        # (parent_joint_name,
        #  parent_link_name) = robot_model.parent_map[root.name]
        # parent_joint = robot_model.joint_map[parent_joint_name]

        # # construct the kdl segment
        # sgm = kdl.Segment(
        #     root.name,
        #     self._toKdlJoint(parent_joint),
        #     self._toKdlPose(parent_joint.origin),
        #     inert)
        ...
        for child in children:
            if not self._add_children_to_tree(robot_model, child, tree):
                return False
